
StoryState = Class{__includes = BaseState}

function StoryState:init()

end

function StoryState:update(dt)
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        gSounds['button_pressed']:play()
        gStateMachine:change('play')
    end
end

function StoryState:render()
    love.graphics.draw(gTextures['game_background'], 0, 0, 0, 
    VIRTUAL_WIDTH / gTextures['game_background']:getWidth(),
    VIRTUAL_HEIGHT / gTextures['game_background']:getHeight())

    love.graphics.setColor(26/255, 33/255, 48/255, 0.3)
    love.graphics.rectangle("fill", 50, 50, 280, 140, 10)


    love.graphics.setFont(gFonts['small'])
    love.graphics.setColor(255, 255, 255, 1)

    love.graphics.printf('Lots of quests await you...', 60, VIRTUAL_HEIGHT / 2 - 40, 270, 'center')
    love.graphics.printf('Can you complete them all?', 60, VIRTUAL_HEIGHT / 2 , 270, 'center')
    
    love.graphics.setFont(gFonts['small'])
    love.graphics.setColor(26/255, 33/255, 48/255, 1)
    love.graphics.printf('Press Enter to continue...', 60, VIRTUAL_HEIGHT - 50 , 270, 'center')
end